...
<div class="col-md-12 ftco-animate opacidad">
    <?php
        $icono = get_template_directory_uri().'/images/details-unavailable.svg';  
        if(get_post_meta($post->ID, 'disponibleModel', true)){
            $icono = get_template_directory_uri().'/images/details-available.svg';
        };
        if(get_post_type() == "models_hiring"){
            echo '<div class="blog-entry">';
            echo '<img class="custommodelos" src="';
            echo $icono.'"/>';
        }else{
            echo '<div class="blog-entry">';
        }
    ?>
        ...
        <div class="desc pl-3 mt-3">
                <?php 
                    if(get_post_type() == "models_hiring"){
                        $titulo = get_post_meta($post->ID, 'nombreModel', true)." ".get_post_meta($post->ID, 'apellidosModel', true);
                ?>
                        <h3 class="heading"><a href="<?php the_permalink(); ?>"><?php echo $titulo; ?></a></h3>
                <?php
                    }else{
                ?>
